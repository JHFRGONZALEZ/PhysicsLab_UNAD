...
export const VisualizationCanvasMRUV: React.FC = () => {
  ...
  const draw = useCallback(() => {
    ...
    // Car body (rocket shape)
    const carGrad = ctx.createRadialGradient(carX - 5, carY - 5, 2, carX, carY, carRadius);
    carGrad.addColorStop(0, '#c084fc');
    carGrad.addColorStop(1, '#7e22ce');
    ctx.beginPath();
    ...
    ctx.textAlign = 'center';
    ctx.fillText('🚀', carX, carY + 7);

    // Velocity vector
    if (showVelocityVector && velocity !== 0) {
      const arrowLength = velocity * 10;
    ...
    if (showAccelerationVector && acceleration !== 0) {
      const arrowLength = acceleration * 15;
      const arrowStartX = carX + (acceleration > 0 ? carRadius + 5 : -carRadius - 5);
      const arrowEndX = arrowStartX + arrowLength;
      const arrowY = carY + carRadius + 20;
      ...
      ctx.fillStyle = '#16a34a';
      ctx.textAlign = 'center';
      ctx.fillText(`a = ${acceleration.toFixed(1)} m/s²`, (arrowStartX + arrowEndX) / 2, arrowY + 20);
    }
    ...
    ctx.fillText(`${velocity.toFixed(2)} m/s`, width - 20, 108);
    ...
    ctx.textAlign = 'center';
    ctx.fillText(
      `x(t) = ${initialPosition.toFixed(1)} + ${initialVelocity.toFixed(1)}t + ½(${acceleration.toFixed(1)})t²`,
      width / 2,
      height - 15
    );
  ...
  }, [position, velocity, acceleration, time, initialPosition, initialVelocity, showVelocityVector, showAccelerationVector, showTrail, trailPoints, isRunning]);
  ...
  return (
    <canvas
      ...
      width={800}
      height={300}
      className="w-full rounded-xl shadow-lg border border-purple-200 bg-white"
